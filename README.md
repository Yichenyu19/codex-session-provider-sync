...
> 切换 Codex 的 `model_provider` 之后，点一下桌面快捷方式，把左边还在显示的未归档会话同步回来。

这不是常驻应用，也不是后台服务。它就是一个 Windows 小工具，帮你把本地 Codex 的会话元数据对齐到当前供应商，尽量别让历史突然“消失”。

## 它能做什么

- 读取 `%USERPROFILE%\.codex\config.toml` 里的当前 `model_provider`。
...
以后你只要在切换 Codex 供应商后，双击桌面快捷方式就行。窗口会告诉你现在对齐到哪个供应商、改了多少、备份放哪了。

如果 Codex 正在写状态导致数据库被锁住，先关掉 Codex，再点一次。
...
Codex Session Provider Sync is a small Windows helper for Codex Desktop. When you switch providers, you can click once and sync your unarchived local sessions back to the current `model_provider`.
